...
#include "FEM-Cubed_config.hpp"
#include "FEM3_EquationSetFactory.hpp"
#include "FEM3_BCStrategy_Factory.hpp"
#include "FEM3_ClosureModel_Factory_TemplateBuilder.hpp"
#include "FEM3_AddFieldsToMesh.hpp"
#include "FEM3_OperatorRequestCallback.hpp"
#include "FEM3_FullMaxwellPreconditionerFactory.hpp"
#include "FEM3_DiscreteGradient.hpp"

#include <string>
#include <iostream>


Teuchos::RCP<Teuchos::ParameterList> maxwellParameterList(const int basis_order, const std::string pb_model);
...
Teuchos::RCP<Teuchos::ParameterList> auxOpsParameterList(const int basis_order, const double massMultiplier, const std::string pb_model);
...
int main_(Teuchos::CommandLineProcessor &clp, int argc,char * argv[])
{
...

  {
    ...
    clp.setOption("mu",&mu);
    clp.setOption("contrast",&contrast);
    ...
    double epsilon1 = contrast*epsilon;
    double mu1 = contrast*mu;

    if (use_stacked_timer)
      stacked_timer = rcp(new Teuchos::StackedTimer("FEM3"));
    Teuchos::TimeMonitor::setStackedTimer(stacked_timer);


    Teuchos::TimeMonitor tM(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: Total Time")));
    ...
    Teuchos::RCP<panzer_stk::STK_MeshFactory> mesh_factory;
    {
      Teuchos::TimeMonitor tMmesh(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: build mesh")));
    ...
    *out << "dt:      " << dt << std::endl << std::endl;
    ...
    // factory definitions
    Teuchos::RCP<FEM3::EquationSetFactory> eqset_factory =
      Teuchos::rcp(new FEM3::EquationSetFactory(auxGlobalData)); // where the maxwell equations are defined
    FEM3::BCFactory bc_factory;                                  // where boundary conditions are defined
    ...
    Teuchos::RCP<Teuchos::ParameterList> physicsBlock_pl0 = maxwellParameterList(basis_order, "electromagnetics0");
    Teuchos::RCP<Teuchos::ParameterList> physicsBlock_pl1 = maxwellParameterList(basis_order, "electromagnetics1");
    ...
    {
      Teuchos::TimeMonitor tMphysics(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: build physics blocks")));

      bool build_transient_support = true;

      std::vector<std::string> block_names;
      mesh->getElementBlockNames(block_names);
      const panzer::CellData volume_cell_data0(workset_size, mesh->getCellTopology(block_names[0]));
      const panzer::CellData volume_cell_data1(workset_size, mesh->getCellTopology(block_names[1]));
      ...
      RCP<panzer::PhysicsBlock> pb0
        = rcp(new panzer::PhysicsBlock(physicsBlock_pl0,
                                       block_names[0],
      ...
      RCP<panzer::PhysicsBlock> pb1
        = rcp(new panzer::PhysicsBlock(physicsBlock_pl1,
                                       block_names[1],
                                       default_integration_order,
                                       volume_cell_data1,
                                       eqset_factory,
      ...
      physicsBlocks.push_back(pb0);
      physicsBlocks.push_back(pb1);
    }

    // build the auxiliary physics blocks objects
    Teuchos::RCP<Teuchos::ParameterList> auxPhysicsBlock_pl0;
    Teuchos::RCP<Teuchos::ParameterList> auxPhysicsBlock_pl1;
    if (use_refmaxwell) {
      auxPhysicsBlock_pl0 = auxOpsParameterList(basis_order, mu/dt, "electromagnetics_aux0");
      auxPhysicsBlock_pl1 = auxOpsParameterList(basis_order, mu/dt, "electromagnetics_aux1");
    } else {
      auxPhysicsBlock_pl0 = auxOpsParameterList(basis_order, 1.0, "electromagnetics_aux0");
      auxPhysicsBlock_pl1 = auxOpsParameterList(basis_order, 1.0, "electromagnetics_aux1");
    }
    std::vector<RCP<panzer::PhysicsBlock> > auxPhysicsBlocks;
    {
      Teuchos::TimeMonitor tMaux_physics(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: build auxiliary physics blocks")));
      ...
      // the physics block knows how to build and register evaluator with the field manager
      RCP<panzer::PhysicsBlock> pb0
        = rcp(new panzer::PhysicsBlock(auxPhysicsBlock_pl0,
                                       block_names[0],
                                       default_integration_order,
                                       volume_cell_data0,
                                       eqset_factory,
                                       ...
                                       build_transient_support));
      RCP<panzer::PhysicsBlock> pb1
        = rcp(new panzer::PhysicsBlock(auxPhysicsBlock_pl1,
                                       block_names[1],
                                       default_integration_order,
                                       volume_cell_data1,
                                       eqset_factory,
      ...
      // we can have more than one physics block, one per element block
      auxPhysicsBlocks.push_back(pb0);
      auxPhysicsBlocks.push_back(pb1);
    }
    ...
    FEM3::ClosureModelFactory_TemplateBuilder cm_builder;
    cm_factory.buildObjects(cm_builder);
    Teuchos::ParameterList closure_models("Closure Models");
    {
      // current source
      closure_models.sublist("electromagnetics0").sublist("CURRENT").set<std::string>("Type","GAUSSIAN PULSE"); // a gaussian current source
      closure_models.sublist("electromagnetics0").sublist("CURRENT").set<double>("dt",dt); // set pulse width such that dt resolves it
      closure_models.sublist("electromagnetics1").sublist("CURRENT").set<std::string>("Type","GAUSSIAN PULSE"); // a gaussian current source
      closure_models.sublist("electromagnetics1").sublist("CURRENT").set<double>("dt",dt); // set pulse width such that dt resolves it
      ...
      closure_models.sublist("electromagnetics0").sublist("PERMITTIVITY").set<double>("Value",epsilon0);
      closure_models.sublist("electromagnetics0").sublist("INVERSE_PERMEABILITY").set<double>("Value",1.0/mu0);
      closure_models.sublist("electromagnetics0").sublist("CONDUCTIVITY").set<double>("Value",0.0);
      closure_models.sublist("electromagnetics1").sublist("PERMITTIVITY").set<double>("Value",epsilon1);
      closure_models.sublist("electromagnetics1").sublist("INVERSE_PERMEABILITY").set<double>("Value",1.0/mu1);
      closure_models.sublist("electromagnetics1").sublist("CONDUCTIVITY").set<double>("Value",0.0);
      closure_models.sublist("electromagnetics_aux0").sublist("PERMITTIVITY").set<double>("Value",epsilon0);
      closure_models.sublist("electromagnetics_aux0").sublist("INVERSE_PERMEABILITY").set<double>("Value",1.0/mu0);
      closure_models.sublist("electromagnetics_aux1").sublist("PERMITTIVITY").set<double>("Value",epsilon1);
      closure_models.sublist("electromagnetics_aux1").sublist("INVERSE_PERMEABILITY").set<double>("Value",1.0/mu1);

      // electromagnetic energy: 1/2 * ((E, epsilon * E) + (B, 1/mu * B))
      closure_models.sublist("electromagnetics0").sublist("EM_ENERGY").set<std::string>("Type","ELECTROMAGNETIC ENERGY");
      closure_models.sublist("electromagnetics1").sublist("EM_ENERGY").set<std::string>("Type","ELECTROMAGNETIC ENERGY");
    }

    Teuchos::ParameterList user_data("User Data"); // user data can be empty here

    // add full maxwell solver to teko
    RCP<Teko::Cloneable> clone = rcp(new Teko::AutoClone<FEM3::FullMaxwellPreconditionerFactory>());
    ...
    req_handler->addRequestCallback(Teuchos::rcp(new FEM3::OperatorRequestCallback(auxGlobalData, matrix_output)));
    ...
    {
      Teuchos::TimeMonitor tMdiscGrad(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: add discrete gradient")));
    ...
    {
      Teuchos::TimeMonitor tMphysicsEval(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: setup physics model evaluator")));
    ...
    {
      Teuchos::TimeMonitor tMauxphysicsEval(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: setup auxiliary physics model evaluator")));
    ...
    {
      Teuchos::TimeMonitor tMts(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: timestepper")));
      for(int ts = 1; ts < numTimeSteps+1; ts++)
        {
          ...
          if (exodus_output)
            {
              Teuchos::TimeMonitor tMexodus(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM3: timestepper: writeToExodus")));
...
void createExodusFile(const std::vector<Teuchos::RCP<panzer::PhysicsBlock> >& physicsBlocks,
  ...
  for(std::size_t i=0;i<physicsBlocks.size();i++) {
    ...
    FEM3::addFieldsToMesh(*mesh,output_pl);
  }
