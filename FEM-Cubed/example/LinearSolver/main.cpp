...
//#include "FEM3_RefMaxwellPreconditionerFactory.hpp"
...
int main(int argc,char * argv[])
{
  ...
  { // Put main inside braces to delete all views prior to finalize of device
    ...
    {
      Teuchos::RCP<Teuchos::TimeMonitor> tM = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM-Cubed: Total Time"))));
      ...
      //clone = rcp(new Teko::AutoClone<FEM3::RefMaxwellPreconditionerFactory>());
      ...
      {
        Teuchos::RCP<Teuchos::TimeMonitor> tM = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM-Cubed: timestepper"))));
        for(int ts = 1; ts < numTimeSteps+1; ts++)
        {
          ...
          if (exodus_output)
          {
            Teuchos::RCP<Teuchos::TimeMonitor> tM = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FEM-Cubed: timestepper: writeToExodus"))));
