#ifndef FEM3_GAUSSIAN_PULSE_IMPL_HPP
#define FEM3_GAUSSIAN_PULSE_IMPL_HPP
  ...
  if (ir_dim == 3) {
    for (index_t cell = 0; cell < workset.num_cells; ++cell) {
      for (int point = 0; point < current.extent_int(1); ++point) {
        ...
        const ScalarT  r2 = (x-0.0)*(x-0.0) + (y-0.0)*(y-0.0) + (z-0.0)*(z-0.0);
        current(cell,point,0) = 0.0;
        current(cell,point,1) = 0.0;
        //current(cell,point,2) = std::exp(-r2*scale)*factor;
        current(cell,point,2) = std::exp(-r2*r2/alpha/alpha)*((time-2.0*beta)/beta/beta)*std::exp(-(time-2.0*beta)*(time-2.0*beta)/beta/beta);
      }
    }
  } else {
    for (index_t cell = 0; cell < workset.num_cells; ++cell) {
